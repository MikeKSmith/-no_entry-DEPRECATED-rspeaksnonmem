...
## getting started
`rspeaksnonmem` needs to know where NONMEM and PsN are installed on your system.
The function defineExecutable takes this information and creates the system command line for each tool.
Typically PsN functions are called by calling Perl with the appropriate PsN Perl script.
If the user has NONMEM and Perl setup on their system PATH then the "path" argument can be left blank.
This will mean that the "command" is sufficient to define how to call each tool. On Linux systems the 
"path" argument should be left blank.
...
installPaths <- c("c:/nm72", rep(perlPath,5))
tools <- c("NONMEM", "execute", "VPC", "bootstrap", "SSE", "runrecord")
