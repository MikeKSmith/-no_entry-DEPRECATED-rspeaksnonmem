...
```{r defineWorkflow}
myPopPKWorkflow <- function(model, workingDir=NULL){
  if(is.null(workingDir)) workingDir <- file.path(getwd(),deparse(substitute(model)))
  dir.create(workingDir)

  file.copy(from = Model1$Data[,"File"], to = workingDir)
  ...
  writeNMControlStream(templateModel = warfModel$Raw, parsedControl = Model1,
                       outputFile = controlFile)
  
  estimate_NM(modelFile = controlFile, reportExtension = "lst", 
              command = "nonmem-7.2",
              working.dir = workingDir, cleanup = T)
              
  sumo_PsN(command = "sumo-3.5.4", listFile = lstFile,
           working.dir = workingDir)
  
  basicGOF_Xpose( working.dir = workingDir )
