...
```{r defineModels}
  Model1 <- updateModel(warfTemplateModel, theta=thetaNoLag, omega=diagCLV_noKATLAG, runno=1) ## First model
  Model2 <- updateModel(warfTemplateModel, theta=thetaLag, omega=diagCLV_noKATLAG,   runno=2)  ## Add POP_TLAG
  Model3 <- updateModel(warfTemplateModel, theta=thetaLag, omega=diagPPV_noTLAG,     runno=3)  ## Add PPV_KA
  Model4 <- updateModel(warfTemplateModel, theta=thetaLag, omega=CLVBlock_noTLAG,    runno=4)  ## Change to block CL, V
  Model5 <- updateModel(warfTemplateModel, theta=thetaLag, omega=diagPPV,            runno=5)  ## Add PPV_TLAG
  Model6 <- updateModel(warfTemplateModel, theta=thetaLag, omega=CLVKABlock,         runno=6)  ## Change to block CL, V, KA
  Model7 <- updateModel(warfTemplateModel, theta=thetaLag, omega=fullBlockOmega,     runno=7)  ## full block OMEGA
  Model8 <- updateModel(warfTemplateModel, theta=thetaLagComboRUV, omega=fullBlockOmega, runno=8)  ## Combined RUV model (add + prop) 
```
...
myPopPKWorkflow <- function(model){
  ...
  writeNMControlStream(templateModel = warfModel$Raw, parsedControl = model,
                       outputFile = controlFile, outFileExtension = "ctl")
  
  execute_PsN(modelFile = controlFile, command="execute-3.5.4", 
