...
```{r NONMEM }
estimate_NM(modelFile="theophylline.mod", installInfo=installedSoftware, cleanup=T)
```
## Returning results from NONMEM output 
```{r Sumo }
sumo_PsN(listFile="theophylline.lst", installInfo=installedSoftware)
```
...
```{r executePsN, include=FALSE}
execute_PsN(modelFile='theophylline.mod', command="execute-3.5.4", 
            addargs=list(tweak_inits=TRUE, retries=3), working.dir="Run1")
```
## VPC 
Next we can perform VPC using PsN:
```{r VPC_PsN}
VPC_PsN(modelFile='theophylline.mod', installInfo=installedSoftware, working.dir="Run1_VPC",
              nsamp=100, seed=54321, 
              addargs=list(lst="theophylline.lst", bin_by_count=0,
