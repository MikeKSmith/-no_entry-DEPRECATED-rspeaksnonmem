...
```{r NONMEM }
estimate_NM(modelFile = "theophylline.mod", command="c:/nm72/run/nmfe72.bat")
```
## Returning results from NONMEM output 
```{r Sumo}
getwd()
sumo_PsN(lstFile = "theophylline.lst", installPath = perlPath, version=PsNVersion)
```
...
```{r defineExecutable2}
defineExecutable(tool = "nonmem", installPath = nmPath, searchCommand = T)
```
...
```{r executePsN}
execute_PsN(modelFile='theophylline.mod',
...
```{r VPC_PsN}
VPC_PsN( modelFile='theophylline.mod', 
         installPath = perlPath,
         version = PsNVersion,
