...
#' @examples
runRecord_PsN <- function(command = NULL, to = NULL, runRoot = "Run", modelExtension = ".mod", 
                          outputExtension = ".lst", addargs = NULL, cleanup = T, working.dir = NULL, ...) {
  
  working.dir <- ifelse(is.null(working.dir), getwd(), working.dir)
  
  modelFiles <- list.files(pattern = paste(runRoot, "[0-9]\\", modelExtension, sep = ""), 
                           recursive = T)
  lstFiles <- list.files(pattern = paste(runRoot, "[0-9]\\", outputExtension, sep = ""), 
                         recursive = T)
  ...
  runRecord <- read.table("AAruninfo.txt", sep = ";", row.names = NULL, skip = 5, header = F, 
