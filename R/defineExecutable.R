...
#' @param installInfo A named list or data frame containing path and command 
#' line string / shortcut.
#' List names should match software names. Acts as a lookup table for specifying 
#' commands.
...
defineExecutable <- function(tool = NULL, installInfo = NULL, command = NULL, 
                             installPath = NULL, searchCommand = FALSE, ...) {
    ...
    if (is.null(installInfo)) {
        if (!searchCommand) {
            ...
            command <- dir(path = installPath, pattern = command, recursive = T, 
                full.names = T)
        }
        return(command)
    }
    ...
    if (!is.null(installInfo)) {
        whichSoftware <- casefold(installInfo$tool, upper = T) == casefold(tool, 
            upper = T)
        ...
        if (!is.na(mySoftware$path)) 
            if (casefold(tool, upper = T) == "NONMEM") {
                if (mySoftware$path != "") {
                  ...
                  mySoftwareCall <- paste(file.path(myPath, mySoftware$command), 
                    ".bat", sep = "")
        ...
        if (casefold(tool, upper = T) != "NONMEM") {
            ...
            mySoftwareCall <- paste(file.path(myPath, "perl"), " ", 
                                    file.path(myPath,mySoftware$command), 
                                    ".pl", sep = "")
            return(mySoftwareCall)
        }
