...
getNMDataObjects <- function(RNMImportObject, problemNo=1) {
  ...
  rowsData <- data.frame( blockInfo[grep("DAT",as.character(blockInfo$Search)), c("firstRow", "nextBlockRow")] , row.names=NULL)
  rowsInput <- data.frame( blockInfo[grep("INP",as.character(blockInfo$Search)), c("firstRow", "nextBlockRow")] , row.names=NULL)
...
getNMParameterObjects <- function(RNMImportObject, problemNo=1) {
  ...
  ## What rows correspond to each parameter type  
  rowsTheta <- data.frame( blockInfo[grep("THE",as.character(blockInfo$Search)), c("firstRow", "nextBlockRow")] , row.names=NULL)
  rowsOmega <- data.frame( blockInfo[grep("OME",as.character(blockInfo$Search)), c("firstRow", "nextBlockRow")] , row.names=NULL)
  rowsSigma <- data.frame( blockInfo[grep("SIG",as.character(blockInfo$Search)), c("firstRow", "nextBlockRow")] , row.names=NULL)
...
getNMTaskPropertiesObjects <- function(RNMImportObject, problemNo=1) {
  ...
  ## What rows correspond to each parameter type  
  rowsEST <- data.frame( blockInfo[grep("EST",as.character(blockInfo$Search)), c("firstRow", "nextBlockRow")] , row.names=NULL)
  rowsTAB <- data.frame( blockInfo[grep("TAB",as.character(blockInfo$Search)), c("firstRow", "nextBlockRow")] , row.names=NULL)
  rowsCOV <- data.frame( blockInfo[grep("COV",as.character(blockInfo$Search)), c("firstRow", "nextBlockRow")] , row.names=NULL)
  
  ## Extract EST rows (if there are any)
  rawEstRows <- NULL 
  if ( length( grep( "EST", as.character( blockInfo$Search ) ) ) > 0 ){
    rawEstRows <- apply( rowsEST, 1, function(x) Raw[ x[1] : if(x[2]>x[1])(x[2] - 1) else (x[2]) ] )
  }
  
  ## Extract TAB rows (if there are any)
  rawTabRows <- NULL
  if ( length( grep( "TAB", as.character( blockInfo$Search ) ) ) > 0 ) {
    rawTabRows <- apply( rowsTAB, 1, function(x) Raw[ x[1] : if(x[2]>x[1])(x[2] - 1) else (x[2]) ] )
  }
  
  ## Extract COV rows (if there are any)
  rawCovRows <- NULL
  if ( length( grep( "COV", as.character( blockInfo$Search ) ) ) > 0 ){
    rawCovRows <- apply( rowsCOV, 1, function(x) Raw[ x[1] : if(x[2]>x[1])(x[2] - 1) else (x[2]) ] )
  }
