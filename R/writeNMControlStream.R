...
writeNMControlStream <- function(templateModel, parsedControl, outputFile, outFileExtension = ".mod", 
  ...
  probLines <- sapply(problems, function(x)match(x,sub("^\\$\\w* ","",raw,perl=T)))
  
  ### Handling >1 Problem
  ##  Match raw Problem statement with parsed Problem statement then complete the actions below:
  ...
  for(bb in 1:length(parsedControl)){
    ...
    if(!is.null(control2$Omega)){
      ...
      for (i in 1:length( control2$Omega ) ){
        ...
        if ( !is.null( x$block ) ){
          ...
          omegaBlockName <- ifelse(length(grep("Omega",ctrlmerged$RNMI.block))>0, 
                                   ctrlmerged$orig.block[grep("Omega",ctrlmerged$RNMI.block)],
                                   "")
    ...
    if(!is.null(control2$Sigma)){
      ...
      for (i in 1:length( control2$Sigma ) ){
        ...
        if ( !is.null( x$block ) ){
          ...
          out <- apply(y, 1, paste, collapse=" ")
          sigmaBlockName <- ifelse(length(grep("Sigma",ctrlmerged$RNMI.block))>0, 
                                   ctrlmerged$orig.block[grep("Sigma",ctrlmerged$RNMI.block)],
                                   "")
    ...
    dataBlockName <- ifelse(length(grep("Data",ctrlmerged$RNMI.block))>0, 
                            ...
                            "")
    control2$Data <- paste( paste("$",dataBlockName,sep="")," ",
                            paste(Data, collapse=" ")
    )
    ...
    probBlockName <- ifelse(length(grep("Problem",ctrlmerged$RNMI.block))>0, 
                            ctrlmerged$orig.block[grep("Problem",ctrlmerged$RNMI.block)],
                            "")
    ...
    if(bb==1)sink(file = fileName) else sink(file = fileName, append=TRUE)
    for (i in 1:nrow(ctrlmerged)) {
      cat(paste("\n",control3[[i]]))
    }
    sink()
